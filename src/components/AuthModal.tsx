...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const isAdminUser = lowerEmail === 'vanessadgustarconfeitaria@admin.com' || 
      ...
      if (isAdminUser) {
        ...
        toast.success('Acesso administrativo confirmado via Google!');
      } else {
        // Standard user - check if exists, if not create record
        await setDoc(doc(db, 'users', user.uid), {
          uid: user.uid,
      ...
      if (error.code === 'auth/popup-blocked') {
        toast.error('O pop-up de login foi bloqueado pelo seu navegador.');
      } else {
  ...
  const handleAuth = async () => {
    ...
    if (!trimmedEmail || !trimmedPassword) {
      toast.error('Preencha os campos obrigatórios (Email e Senha)');
      return;
    }
            ...
            try {
              ...
              return;
               ...
               if (createErr.code === 'auth/email-already-in-use') {
                 throw new Error('Este e-mail já possui uma conta com outra senha. Use o botão "Entrar com Google" para acessar imediatamente.');
               }
               throw createErr;
            }
          }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[400px]">
        ...
        <div className="space-y-4 py-4">
            ...
            <div className="flex justify-between items-center">
              ...
              {isLogin && (
                <button 
                  ...
                  className="text-[10px] text-pink-600 font-bold hover:underline uppercase tracking-tight"
                  disabled={loading}
                >
                  Esqueci Senha
                </button>
          ...
          <Button 
            variant="outline"
            className="w-full border-gray-300 hover:bg-gray-50 h-10 gap-2"
